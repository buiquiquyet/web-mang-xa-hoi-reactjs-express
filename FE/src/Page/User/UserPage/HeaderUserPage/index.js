...
import {
  setMessItem,
  zoomOutHideToMessItem,
} from "../../../../Reducer/useReducerMessager/actions";
...
function HeaderUserPage({ userId }) {
  ...
  const handleAcceptedAddFriend = async (userId1, userId2) => {
    const rs = await ServiceFriendApi.updateAcceptedFriend({
      userId1,
      userId2,
    });
    if (rs.success) {
  ...
  const renderStatusFriend = (statusFriend, dataUser, userId) => {
    if (statusFriend) {
      if (statusFriend.status === "pending") {
        return dataUser._id === statusFriend.receiver_id ? (
          <div
            className={cx("option-editSefl")}
            onClick={() => handleAcceptedAddFriend(dataUser._id, userId)}
          >
          ...
          <div
            className={cx("option-editSefl")}
            onClick={() => handleCancelAddFriend(dataUser._id, userId)}
          >
    ...
    return (
      <div
        className={cx("option-editSefl")}
        onClick={() => handleAddFriend(dataUser._id, userId)}
      >
  ...
  useEffect(() => {
    if (userId) {
      const fecthImageAvartarCover = async (userId) => {
        const imageAvartar = await ServicePostApi.showPostByUserAvartarCover({
          typePost: "avartar",
          userId: userId,
        });
        const imageCover = await ServicePostApi.showPostByUserAvartarCover({
          typePost: "cover",
          userId: userId,
        });
          ...
          <div className={cx("header-coverImg")}>
            <div className={cx("coverImg")}>
              {Object.keys(imageCover).length > 0 ? (
                <img
                  src={ImageUrlPath + imageCover.url}
                  alt="img"
                  onClick={() => handleShowImageSlide(imageCover)}
                />
              ) : (
              ...
              {dataUser && dataUser._id === userId && (
                <div
                  className={cx("header-addCoverImg")}
                  onClick={() => handleShowHideModal("cover")}
                >
            ...
            <div className={cx("avatar-person")}>
              <div className={cx("avatar-div")}>
                <div className={cx("avatar-img")}>
                  <img
                    src={
                      Object.keys(imageAvartar).length > 0
                ...
                {dataUser && dataUser._id === userId && (
                  <div
                    ...
                    onClick={() => handleShowHideModal("avartar")}
                  >
                    <div className={cx("cameraIcon")}></div>
                  </div>
                )}
              ...
              <div className={cx("avatar-info")}>
                <div className={cx("avatar-name")}>
                  {fullNameUser &&
                    fullNameUser.firstName + " " + fullNameUser.lastName}
                </div>
                <span className={cx("avatar-friend")}>
                  {" "}
                  {totalFriend && totalFriend} bạn bè
                </span>
              </div>
            </div>
              ...
              <div className={cx("option-addEdit")}>
                {dataUser && dataUser._id === userId ? (
                  ...
                  <>
                    ...
                    <div
                      className={cx("option-addFeed")}
                      onClick={() => handleShowMessLogorShowMess(userId)}
                    >

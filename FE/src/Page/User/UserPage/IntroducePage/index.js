...
function IntroducePage() {
        ...
        return (
                <div  className={cx('right-input')} >
                    ...
                    <div className={cx('input-button')}>
                        ...
                        <button 
                            className={cx({'activeBtn': dataIntrocudeInput[idInput] !== null
                                                        && (dataIntrocudeInput.hasOwnProperty(idInput) 
                                                        && dataIntrocudeInput[idInput].length > 0)
                                                        }, 'disbled')}
                            onClick={() => {
                                if (dataIntrocudeInput.hasOwnProperty(idInput) && dataIntrocudeInput[idInput].length > 0) {
    ...
    const renderInfoIntroduce = ( dataIntrocudeInputApi,idInput, imgUrl, textItem1, textItem2) => {
        return (
                            ...
                            <HeadelessTippy
                                render={attrs => (
                                    <div className={cx('option-select')} tabIndex="-1" {...attrs}>
                                        <div className={cx('select-item')} onClick={() => handleSetStatusInput(idInput)}>
                                            <LogoEdit className={cx('icon-select')}/>
                                            <span >Sửa {textItem2}</span>
                                        </div>
                                        <div className={cx('select-item')} onClick={() => handleDelIntroduce(userId, idInput)}  >
            ...
            :
                ...
                :
                <div className={cx('item-label')} onClick={() => handleSetStatusInput(idInput)}>
           ...
           <div className={cx('app')}>
            <div className={cx('left-introduce')}>
                ...
                <div className={cx('right-introduce')}>
                    ...
                    {
                        [
                            { key: 'school', img: schoolImg, text1: 'Đã học tại', text2: 'trường học' },
                            { key: 'province', img: homeImg, text1: 'Sống tại', text2: 'tỉnh/thành phố hiện tại' },
                            { key: 'place', img: locationImg, text1: 'Đến từ', text2: 'nơi sống hiện tại' },
                            { key: 'status', img: statusImg, text1: '', text2: 'tình trạng mối quan hệ hiện tại' },
                            { key: 'phoneNumber', img: phoneNumberImg, text1: '', text2: 'số điện thoại cá nhân' }
                        ].map((item, index) => {
                            return (
                                dataUser && dataUser._id !== userId 
                                ?
                                    <React.Fragment key={index}>
                                        {renderInfoIntroduce( dataIntrocudeInputApi, item.key, item.img, item.text1, item.text2)}
                                    </React.Fragment>
                                :
